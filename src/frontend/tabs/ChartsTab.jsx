...
export default function ChartsTab({
  C, ewk, savSorted, catTot, typTot, snapshots, setSnapshots,
  tNW, tDW, tExpW, tSavW, remW, totalSavPlusRemW,
  chartWeeks, setChartWeeks, includeEaip, setIncludeEaip,
  savRateBase, setSavRateBase, chartOrder, setChartOrder, dragChart, setDragChart,
  catHistMode, setCatHistMode, catHistoryName, setCatHistoryName,
  itemHistMode, setItemHistMode, itemHistoryName, setItemHistoryName,
  necDisMode, setNecDisMode,
  snapDate, setSnapDate, snapLabel, setSnapLabel,
  snapHistView, setSnapHistView, snapHistYear, setSnapHistYear,
  restoreConfirm, setRestoreConfirm,
  p1Name, p2Name, mob, evalF, cSal, kSal, fil, cEaip, kEaip,
  preDed, postDed, c4pre, c4ro, k4pre, k4ro, cHsaAnn, kHsaAnn,
  exp, sav, cats, tax,
  setViewingSnap, setTab, onRestore,
}) {
  ...
  return (
    <div>
      ...
      {(() => {
        ...
        const selCat = catListH.includes(catHistoryName) ? catHistoryName : catListH[0] || "";
        const selItem = namesH.includes(itemHistoryName) ? itemHistoryName : namesH[0] || "";
        ...
        const curEaipNetH = C.eaipNet, curEaipGrossH = C.eaipGross;
        const sCatD = hPts.map(sn => { const row = { date: sn.date, label: sn.label }; if (sn._current) { catListH.forEach(c => { row[c] = Math.round(ewk.filter(e => e.c === c).reduce((sm, e) => sm + e.wk * 48, 0)); }); row._incLine = Math.round(savRateBase === "gross" ? (C.cw + C.kw) * chartWeeks + (includeEaip ? C.eaipGross : 0) : C.net * chartWeeks + (includeEaip ? C.eaipNet : 0)); } else { catListH.forEach(c => { let tot = 0; if (sn.items) Object.values(sn.items).forEach(d => { if (d.c === c && d.t !== "S") tot += d.v || 0; }); row[c] = Math.round(tot); }); const snapEaipN = sn.eaipNet !== undefined ? sn.eaipNet : curEaipNetH; const snapEaipG = sn.eaipGross !== undefined ? sn.eaipGross : curEaipGrossH; row._incLine = Math.round(savRateBase === "gross" ? (sn.grossW || 0) * chartWeeks + (includeEaip ? snapEaipG : 0) : (sn.netW || 0) * chartWeeks + (includeEaip ? snapEaipN : 0)); } return row; });
        const lcp = sCatD[sCatD.length - 1] || {}; const clSorted = [...catListH].sort((a, b) => (lcp[b] || 0) - (lcp[a] || 0));
        const sItemD = hPts.map(sn => { const row = { date: sn.date, label: sn.label }; if (sn._current) { namesH.forEach(n => { const ex = ewk.find(x => x.n === n); const sv = savSorted.find(x => x.n === n); row[n] = Math.round((ex ? ex.wk * 48 : sv ? sv.wk * 48 : 0) * 100) / 100; }); row._incLine = Math.round(savRateBase === "gross" ? (C.cw + C.kw) * chartWeeks + (includeEaip ? C.eaipGross : 0) : C.net * chartWeeks + (includeEaip ? C.eaipNet : 0)); } else { namesH.forEach(n => { row[n] = sn.items?.[n]?.v || 0; }); const snapEaipN = sn.eaipNet !== undefined ? sn.eaipNet : curEaipNetH; const snapEaipG = sn.eaipGross !== undefined ? sn.eaipGross : curEaipGrossH; row._incLine = Math.round(savRateBase === "gross" ? (sn.grossW || 0) * chartWeeks + (includeEaip ? snapEaipG : 0) : (sn.netW || 0) * chartWeeks + (includeEaip ? snapEaipN : 0)); } return row; });
        ...
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={() => setRestoreConfirm(null)}>
          <div onClick={e => e.stopPropagation()} style={{ background: "var(--card-bg, #fff)", borderRadius: 16, padding: 32, maxWidth: 440, width: "90%", boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }}>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
              ...
              <button onClick={() => { onRestore(snapshots[restoreConfirm]); setRestoreConfirm(null); setTab("budget"); }} style={{ padding: "9px 20px", background: "#E8573A", color: "#fff", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Restore</button>
            </div>
          </div>
        </div>
      )}
