  ...
  return (
    <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700, color: "#999", marginLeft: 8 }}>Window:</span>
              {[["all","All"],["10y","10y"],["5y","5y"],["1y","1y"],["ytd","YTD"]].map(([val,lbl]) => (
                <button key={val} onClick={() => setChartTimeWindow && setChartTimeWindow(val)} style={{ padding: "5px 12px", fontSize: 12, fontWeight: 600, border: chartTimeWindow === val ? "2px solid #4ECDC4" : "2px solid var(--bdr, #ddd)", borderRadius: 6, background: chartTimeWindow === val ? "#E8F8F5" : "var(--input-bg, #fafafa)", color: chartTimeWindow === val ? "#4ECDC4" : "#888", cursor: "pointer" }}>{lbl}</button>
              ))}
            </div>
            ...
            {(() => {
              ...
              const sorted = sortedAll.filter(s => {
                ...
                if (winTo && s.date > winTo) return false;
                return true;
              });
              ...
              const chartComponents = {
                pieCategory: dragWrapRender("pieCategory", <Card key={`pc-${chartWeeks}-${includeEaip}-${savRateBase}`}><h3 style={{ margin: "0 0 16px", fontFamily: "'Fraunces',serif", fontSize: 18, fontWeight: 800 }}>By Category <span style={{ fontSize: 12, fontWeight: 500, color: "var(--tx3,#999)" }}>(% of {savRateBase} income, {chartWeeks}wk)</span></h3><div style={{ width: "100%", minHeight: 250 }}><ResponsiveContainer width="100%" height={250}><PieChart><Pie data={catTot} cx="50%" cy="50%" outerRadius={95} innerRadius={48} paddingAngle={2} dataKey="value" stroke="none">{catTot.map((e, i) => <Cell key={i} fill={e.color} />)}</Pie><Tooltip content={<PieTooltip />} /><Legend wrapperStyle={{ fontSize: 11 }} /></PieChart></ResponsiveContainer></div></Card>),
                pieNecDis: dragWrapRender("pieNecDis", <Card key={`pnd-${chartWeeks}-${includeEaip}-${savRateBase}`}><h3 style={{ margin: "0 0 16px", fontFamily: "'Fraunces',serif", fontSize: 18, fontWeight: 800 }}>Necessity vs Discretionary <span style={{ fontSize: 12, fontWeight: 500, color: "#999" }}>(% of {savRateBase} income, {chartWeeks}wk)</span></h3><div style={{ width: "100%", minHeight: 250 }}><ResponsiveContainer width="100%" height={250}><PieChart><Pie data={typTot} cx="50%" cy="50%" outerRadius={95} innerRadius={48} paddingAngle={3} dataKey="value" stroke="none">{typTot.map((e, i) => <Cell key={i} fill={e.color} />)}</Pie><Tooltip content={<PieTooltip />} /><Legend wrapperStyle={{ fontSize: 11 }} /></PieChart></ResponsiveContainer></div></Card>),
                ...
                incomeHistory: (transactions && transactions.length > 0) ? dragWrapRender("incomeHistory", (() => {
                  // Phase 6a: monthly income series from transactions. Uses the
                  // same window toggle as everything else on this tab.
                  const expCatSet = new Set(cats || []);
                  const savCatSet = new Set(savCats || []);
                  const xferCatSet = new Set(transferCats || []);
                  const classOpts = { transferCatSet: xferCatSet, expenseCatSet: expCatSet, savingsCatSet: savCatSet };
                  const incCats = incomeCategories(transactions, classOpts);
                  const filterSet = incomeTypeSel === "all"
                    ? null
                    : new Set(incomeTypeSel === "Uncategorized" ? ["Uncategorized"] : [incomeTypeSel]);
                  const incomeData = buildMonthlyIncomeSeries(transactions, {
                    ...classOpts,
                    from: winFrom, to: winTo,
                    categoryFilter: filterSet,
                  });
                  const winLabel = chartTimeWindow === "all" || !chartTimeWindow ? "All time"
                    : chartTimeWindow === "ytd" ? "YTD"
                    : `Last ${chartTimeWindow}`;
                  const mfmt = v => `$${(v / 1000).toFixed(0)}k`;
                  return (
                    <Card key={`ih-${chartTimeWindow}-${incomeTypeSel}`}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
                        <h3 style={{ margin: 0, fontFamily: "'Fraunces',serif", fontSize: 18, fontWeight: 800 }}>
                          Income History
                          <span style={{ fontSize: 12, fontWeight: 500, color: "var(--tx3,#999)", marginLeft: 8 }}>({winLabel})</span>
                        </h3>
                        <select value={incomeTypeSel} onChange={e => setIncomeTypeSel(e.target.value)} style={{ marginLeft: "auto", border: "2px solid var(--input-border, #e0e0e0)", borderRadius: 8, padding: "6px 10px", fontSize: 13, fontFamily: "'DM Sans',sans-serif", background: "var(--input-bg, #fafafa)" }}>
                          <option value="all">All types (Total)</option>
                          {incCats.map(c => <option key={c} value={c}>{c}</option>)}
                        </select>
                      </div>
                      {incomeData.length === 0 ? (
                        <div style={{ width: "100%", minHeight: 250, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--tx3,#999)", fontSize: 13 }}>
                          No income transactions in this window
                        </div>
                      ) : (
                        <div style={{ width: "100%", minHeight: 250 }}>
                          <ResponsiveContainer width="100%" height={250}>
                            <LineChart data={incomeData}>
                              ...
                              <YAxis tick={{ fontSize: 10 }} axisLine={false} tickLine={false} tickFormatter={mfmt} />
                              <Tooltip formatter={v => fmt(v)} contentStyle={cs} />
                              <Legend wrapperStyle={{ fontSize: 11 }} />
                              {incomeTypeSel === "all" ? (
                                <Line type="monotone" dataKey="Total" stroke="#2ECC71" strokeWidth={2.5} dot={{ r: 4, fill: "#2ECC71" }} name="Total" />
                              ) : (
                                <>
                                  <Line type="monotone" dataKey="Total" stroke="#95A5A6" strokeWidth={1.5} strokeDasharray="5 5" dot={{ r: 3, fill: "#95A5A6" }} name="Total" />
                                  <Line type="monotone" dataKey={incomeTypeSel} stroke="#2ECC71" strokeWidth={2.5} dot={{ r: 4, fill: "#2ECC71" }} name={incomeTypeSel} />
                                </>
                              )}
                            </LineChart>
                          </ResponsiveContainer>
